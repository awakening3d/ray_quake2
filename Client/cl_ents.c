...
void CL_AddPacketEntities (frame_t *frame)
{
	...
	for (pnum = 0 ; pnum<frame->num_entities ; pnum++)
	{
		...
		//888
		if (s1->modelindex>0 && s1->modelindex<100) {
			V_AddLight(s1->origin, 150, s1->modelindex*0.008f, 0.5f, 0.5f, 200);
		}
...
//============
		...
		else if (effects & EF_SPINNINGLIGHTS)
		{
			...
			ent.angles[2] = 180;
			{
				...
				V_AddLight(start, 100, 1, 0, 0, DEFAULT_DLIGHT_RANGE);
			}
		}
		else
		{	// interpolate angles
		...
		if (s1->number == cl.playernum+1)
		{
			...
			if (effects & EF_FLAG1)
				V_AddLight(ent.origin, 225, 1.0, 0.1, 0.1, DEFAULT_DLIGHT_RANGE);
			else if (effects & EF_FLAG2)
				V_AddLight(ent.origin, 225, 0.1, 0.1, 1.0, DEFAULT_DLIGHT_RANGE);
			else if (effects & EF_TAGTRAIL)						//PGM
				V_AddLight(ent.origin, 225, 1.0, 1.0, 0.0, DEFAULT_DLIGHT_RANGE);	//PGM
			else if (effects & EF_TRACKERTRAIL)					//PGM
				V_AddLight(ent.origin, 225, -1.0, -1.0, -1.0, DEFAULT_DLIGHT_RANGE);	//PGM
...
//pmm
		...
		if ( (effects&~EF_ROTATE) )
		{
			if (effects & EF_ROCKET)
			{
				...
				V_AddLight(ent.origin, 200, 1, 1, 0, DEFAULT_DLIGHT_RANGE);
			}
...
//PGM
				if (effects & EF_TRACKER)	// lame... problematic?
				{
					...
					V_AddLight(ent.origin, 200, 0, 1, 0, DEFAULT_DLIGHT_RANGE);
				}
				else
				{
					CL_BlasterTrail (cent->lerp_origin, ent.origin);
...
//PGM
			...
			else if (effects & EF_HYPERBLASTER)
			{
				if (effects & EF_TRACKER)						// PGM	overloaded for blaster2.
					V_AddLight(ent.origin, 200, 0, 1, 0, DEFAULT_DLIGHT_RANGE);		// PGM
				else											// PGM
					V_AddLight(ent.origin, 200, 1, 1, 0, DEFAULT_DLIGHT_RANGE);
			}
			...
			else if (effects & EF_BFG)
			{
				...
				V_AddLight(ent.origin, i, 0, 1, 0, DEFAULT_DLIGHT_RANGE);
			}
			// RAFAEL
			else if (effects & EF_TRAP)
			{
				...
				i = (rand()%100) + 100;
				V_AddLight(ent.origin, i, 1, 0.8, 0.1, DEFAULT_DLIGHT_RANGE);
			}
			else if (effects & EF_FLAG1)
			{
				CL_FlagTrail (cent->lerp_origin, ent.origin, 242);
				V_AddLight(ent.origin, 225, 1, 0.1, 0.1, DEFAULT_DLIGHT_RANGE);
			}
			else if (effects & EF_FLAG2)
			{
				CL_FlagTrail (cent->lerp_origin, ent.origin, 115);
				V_AddLight(ent.origin, 225, 0.1, 0.1, 1, DEFAULT_DLIGHT_RANGE);
			}
//======
//ROGUE
			else if (effects & EF_TAGTRAIL)
			{
				...
				V_AddLight(ent.origin, 225, 1.0, 1.0, 0.0, DEFAULT_DLIGHT_RANGE);
			}
			else if (effects & EF_TRACKERTRAIL)
			{
				if (effects & EF_TRACKER)
				{
					...
					if(vidref_val == VIDREF_GL)
						V_AddLight(ent.origin, intensity, -1.0, -1.0, -1.0, DEFAULT_DLIGHT_RANGE);
					else
						V_AddLight(ent.origin, -1.0 * intensity, 1.0, 1.0, 1.0, DEFAULT_DLIGHT_RANGE);
					}
				else
				{
					CL_Tracker_Shell (cent->lerp_origin);
					V_AddLight(ent.origin, 155, -1.0, -1.0, -1.0, DEFAULT_DLIGHT_RANGE);
				}
			}
			else if (effects & EF_TRACKER)
			{
				...
				// FIXME - check out this effect in rendition
				if(vidref_val == VIDREF_GL)
					V_AddLight(ent.origin, 200, -1, -1, -1, DEFAULT_DLIGHT_RANGE);
				else
					V_AddLight(ent.origin, -200, 1, 1, 1, DEFAULT_DLIGHT_RANGE);
			}
//ROGUE
//======
			...
			else if (effects & EF_IONRIPPER)
			{
				...
				V_AddLight(ent.origin, 100, 1, 0.5, 0.5, DEFAULT_DLIGHT_RANGE);
			}
			// RAFAEL
			else if (effects & EF_BLUEHYPERBLASTER)
			{
				V_AddLight(ent.origin, 200, 0, 0, 1, DEFAULT_DLIGHT_RANGE);
			}
			// RAFAEL
			else if (effects & EF_PLASMA)
			{
				...
				V_AddLight(ent.origin, 130, 1, 0.5, 0.5, DEFAULT_DLIGHT_RANGE);
			}
		}
