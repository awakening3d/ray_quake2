...
trace_t		CL_PMTrace(vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end)
{
	...
	// check against world
	t = CM_BoxTrace(0, start, end, mins, maxs, 0, MASK_PLAYERSOLID);
	if (t.fraction < 1.0)
...
trace_t	CL_PMTrace_thread(int thread, vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end)
{
	trace_t	t;
	...
	rhcount = 0;
	// check against world
	t = CM_BoxTrace(thread, start, end, mins, maxs, 0, MASK_PLAYERSOLID);
	if (t.fraction < 1.0)
		t.ent = (struct edict_s *)1;
